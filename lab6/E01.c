...
int main(){
    int selector, finished, status, lenData;
    comando_e command;
    ...
    5.elencare il ritardo complessivo accumulato dalle corse identificate da un certo codice di tratta[ritardo_tot codice]\n\n"
        ...
        if(command == r_errore || status == 1)   printf("Errore di acquisizione");
    }
...
comando_e leggiComando(){
    ...
    if(strcmp(word, "destinanazione") == 0)    command = r_capolinea;
...
int menuParola(comando_e cmd, t_corsa data[MAX_LENGTH_DATA], int lenData){
    ...
    switch(cmd){
        case r_partenza:        scanf("%s", req.dest);      break;
        case r_capolinea:       scanf("%s", req.start);     break;
    ...
    if(lenResponse > 0){
        if(cmd == r_ritardo_tot){
            ...
            printf("Ritardo complessivo relativo a %s: %d\n", corsa.code, delayTot);

        } else {

            for(i=0 ; i<lenResponse ; i++){
                corsa = data[responseList[i]];
                printf("%s %s %s %d/%d/%d %d\n", corsa.code, corsa.start, corsa.dest, corsa.date[0], corsa.date[1], corsa.date[2], corsa.delay);
            }
...
int query(t_request req, t_corsa data[], int lenData, int res[]){
    int i, indexRes, valid, dsum, rssum, resum;

    for(i=0 , indexRes=0 ; i<lenData ; i++){
        dsum = data[i].date[0]*10000+ data[i].date[1]*100+ data[i].date[2];
        resum =req.dateEnd[0]*10000+req.dateEnd[1]*100+req.dateEnd[2];
        rssum = req.dateStart[0]*10000+req.dateStart[1]*100+req.dateStart[2];
        ...
        if(dsum != 0 &&  (dsum < rssum || dsum > resum))                        valid=0;
...
int handleFileInput(char fileName[], t_corsa data[]){
    int lenData, i;
    char c[MAX_CHAR_CORSA+1], s[MAX_CHAR_CORSA+1], d[MAX_CHAR_CORSA+1];
    int a, m, g, dl;
    FILE *fp;
    ...
    fscanf(fp, "%d", &lenData);
    for(i=0 ; i<lenData ; i++){
        fscanf(fp, "%s %s %s %d/%d/%d %*s%*s %d", c, s, d, &a, &m, &g, &dl);
